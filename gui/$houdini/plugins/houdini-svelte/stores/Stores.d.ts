import type { Stores$input, Stores$result, QueryStore, QueryStoreFetchParams} from '$houdini'

export declare class StoresStore extends QueryStore<Stores$result, Stores$input> {
	/**
	 * ### Route Loads
	 ...
	 * 	const store = new StoresStoreStore()
 ...
 *
 * ```js
 * import { load_Stores } from '$houdini';
 ...
 *   };
 *
 *   return await load_Stores({ event, variables });
 * };
 * ```
 *
 ...
 *
 * ```js
 * import { loadAll, load_Stores } from '$houdini';
 * import type { PageLoad } from './$types';
 *
 * export const load: PageLoad = async (event) => {
 *   const variables = {
 ...
 *     load_Stores({ event, variables }),
 *     // load_ANOTHER_STORE
 *   );
 * };
 * ```
 */
export declare const load_Stores: (params: QueryStoreFetchParams<Stores$result, Stores$input>) => Promise<{Stores: StoresStore}>
